...
const LinkArrow = ({ children, href, className }: LinkArrowProps) => {
  return (
    <Link
      ...
      className={`group flex items-center gap-1 transition-all duration-[150] ease-in-out hover:text-neutral-500 ${
        className ?? ""
      }`}
    >
      {children}
      ...
      {href.startsWith("http") || href.startsWith("mailto") ? (
        <ArrowUpRight
          className={`h-2.5 w-2.5 opacity-0 transition-all duration-[150] ease-in-out group-hover:translate-x-0.5 group-hover:opacity-100`}
          strokeWidth={2.6}
        />
      ) : (
        <ArrowRight
